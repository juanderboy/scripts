...
y, si hace falta, eliminar espectros anomalos antes de continuar.
"""
...
def read_experiment(path: str | Path, delimiter: str = ";") -> Experiment:
    """Read the semicolon-separated spectrophotometry file."""
...
def fit_nonnegative_spectra(absorbance: np.ndarray, c: np.ndarray) -> np.ndarray:
...
def direct_nonnegative_error_for_concentrations(
    ...
    spectra = fit_nonnegative_spectra(experiment.absorbance, c)
    residuals = experiment.absorbance - spectra @ c
    return float(np.linalg.norm(residuals))


def direct_nonnegative_error(k: float, experiment: Experiment, c0: float) -> float:
    """Objective function for the A -> B NNLS fit."""
    if k <= 0:
        return np.inf

    c = concentration_profile_a_to_b(experiment.t, k, c0=c0)
    return direct_nonnegative_error_for_concentrations(c, experiment)
...
def fit_a_to_b_nnls(
    ...
    k = optimize_k(
        lambda trial_k: direct_nonnegative_error(trial_k, experiment, c0),
        k_bounds,
    )
    c = concentration_profile_a_to_b(experiment.t, k, c0=c0)
    spectra = fit_nonnegative_spectra(experiment.absorbance, c)
...
def plot_result(experiment: Experiment, result: FitResult) -> None:
    ...
    axes = fig.subplot_mosaic(
        [
            ...
            ["residuals", "concentrations", "singular"],
        ]
    )
    params_text = ", ".join(
    ...
    fig.suptitle(f"Kinetic fit ({result.method}), {params_text}")
...
def ask_preprocessing_choices(
...
) -> tuple[list[int], str, tuple[float, float] | None, int, tuple[float, float], float]:
    ...
    return drop_indices, baseline_mode, baseline_region, baseline_points, work_range, c0


def preprocess_experiment(args: argparse.Namespace, experiment: Experiment) -> tuple[Experiment, tuple[float, float], float]:
    ...
    return corrected, work_range, c0
...
def main() -> None:
    ...
    corrected, work_range, c0 = preprocess_experiment(args, experiment)
